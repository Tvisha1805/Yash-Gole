...
function VideoCard({ src, title, subtitle }: VideoCardProps) {
  ...
  return (
    <div 
      className="relative w-full aspect-[9/16] rounded-3xl overflow-hidden bg-[#111] border border-white/5 group cursor-pointer shadow-xl transition-all duration-500 hover:border-primary/30"
      onClick={toggleMute}
    >
      <video
        ref={videoRef}
      ...
      {!isMuted && <div className="absolute inset-0 border-4 border-primary/20 rounded-3xl pointer-events-none z-30 transition-all"></div>}
    </div>
  );
}
...
export default function SetsPage() {
  const allVideos = [
    ...
    { src: "/video1.mp4", title: "When Tech Fails", subtitle: "Pune Tech Fest" },
    { src: "/video1.mp4", title: "Marriage vs Engineering", subtitle: "Delhi Comedy Club" },
    { src: "/video1.mp4", title: "The HR Negotiation", subtitle: "Corporate Gig - Bangalore" },
    { src: "/video1.mp4", title: "Parents at the Show", subtitle: "Pune Local Set" },
    { src: "/video1.mp4", title: "Why I Hate LinkedIn", subtitle: "Live from Mumbai Special" },
  ];

  return (
    <main className="min-h-screen bg-[#050505] pt-12 relative overflow-hidden">
      ...
      <div className="max-w-[1500px] mx-auto relative z-10 pt-24 md:pt-32 pb-24 px-6 md:px-12">
        ...
        <div className="mb-14 md:mb-20 flex flex-col items-center md:items-start text-center md:text-left gap-4 md:gap-0 md:flex-row md:justify-between border-b border-white/5 pb-10">
          <div>
             <p className="text-primary font-black uppercase tracking-[0.4em] text-[10px] mb-4">— Watch Full Sets</p>
            <h1 className="text-5xl md:text-6xl lg:text-8xl font-headline font-black text-white tracking-tighter mb-4 uppercase">
              THE <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-[#8C9E00]">SETS.</span>
            </h1>
            <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px] md:text-xs">
              Raw crowds, pure energy. Tap any reel to unmute.
            </p>
          </div>
          
          <Link href="/" className="mt-6 md:mt-0 backdrop-blur-md bg-white/[0.03] border border-white/10 hover:border-primary/50 text-white font-bold px-8 py-4 rounded-xl transition-all shadow-lg hover:-translate-y-1 text-xs uppercase tracking-[0.2em] flex items-center gap-3">
             <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
             Return Home
          </Link>
        </div>

        {/* Massive Column Reels TikTok Style Grid */}
